...
$( document ).ready(function() {
  ...
  cartodb.createVis('map', 'https://observatory.cartodb.com/api/v2/viz/d996e7b2-1c78-11e6-b921-0e31c9be1b51/viz.json', { zoom: 11, center: mapCenter})
    .done(function(map,layers){
      ...
      var clickSubitem = function(){
        $( ".js-result-category li a" ).on( "click", function() {
          ...
          sublayer.setSQL(
            'with stats as( select max(data.'+column_name+'), min(data.'+column_name+') \
                            from '+data_table+')\
            select geom.the_geom_webmercator, \
